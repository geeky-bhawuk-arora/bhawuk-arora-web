...
const Navigation = ({ activeSection, scrollToSection }) => {
  ...
  const navItems = ['home', 'experience', 'projects', 'blog', 'contact'];
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <button
                ...
                className={`capitalize transition-colors duration-200 hover:text-blue-400 relative py-1 ${activeSection === item ? 'text-blue-400' : 'text-gray-300'
                  }`}
              >
                {item}
                {activeSection === item && (
          ...
          <div className="md:hidden py-4 space-y-2 bg-slate-800/50 rounded-lg mx-4 mb-4">
            {navItems.map((item) => (
              <button
                key={item}
                onClick={() => handleNavClick(item)}
                className={`block w-full text-left py-2 px-4 capitalize hover:bg-slate-700 rounded transition-colors ${activeSection === item ? 'text-blue-400 bg-slate-700' : 'text-gray-300'
                  }`}
              >
                {item}
              </button>
